...
export function diagramToSvg(diagram: RepositoryDiagram) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${layout.width}" height="${layout.height}" viewBox="0 0 ${layout.width} ${layout.height}" role="img" aria-label="Data-flow architecture diagram"><defs><marker id="arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#8e9aa2"/></marker></defs><rect width="100%" height="100%" fill="#fafbfc"/>${edges}${nodes}</svg>`;
}

export function diagramToDrawio(diagram: RepositoryDiagram) {
  const layout = layoutDiagram(diagram);
  const nodeMap = new Map(layout.nodes.map((node) => [node.id, node]));
  ...
  return `<?xml version="1.0" encoding="UTF-8"?><mxfile host="tracepath"><diagram name="Repository overview"><mxGraphModel dx="1200" dy="800" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="1600" pageHeight="1200"><root><mxCell id="0"/><mxCell id="1" parent="0"/>${cells}${edges}</root></mxGraphModel></diagram></mxfile>`;
}
